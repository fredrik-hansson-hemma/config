...
%macro set_labels_epj(lib=, vatable=, tabledescription=NULL);
	...
	* VATABLE: Den tabell som ska uppdateras med nya kolumnbeskrivningar.;
	...
	%end;
	%else %if %sysfunc(upcase(&lib)) = CYTODOSP %then %do;
	...
	%end;
	%else %if %sysfunc(upcase(&lib)) = ORBITA01 %then %do;
	...
	%end;
	%else %if %sysfunc(upcase(&lib)) = ORBITAP %then %do;
	...
	%end;
	%else %if %sysfunc(upcase(&lib)) = ORBITS %then %do;
	...
	%end;
	%else %do;
		%put NOTE: set_labels_epj: Hittar inget hårdkodat libname till &lib.. Försöker skapa ett metadatalibname.	;
		%* Egentligen vore det nog snyggare att göra på det här sättet för samtliga libnames ovan, men vågar inte ändra programmet nu i brådskan....	;
		libname &lib meta metaout=all /*metaout=data*/ library="&lib";
	%end;
