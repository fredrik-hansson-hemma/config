...
%macro setrowsecurity(path=,table=);
	...
	* Sökväg till katalog på server där skript lagras.;
	%if %upcase(&env) = BST %then %let filepath=/SASWORK/scripts/;
	%else %if %upcase(&env) = BS %then %let filepath=/saswork/scripts/;
	...
	%put &=metaserver &=metaport &=metauser;
	...
	* Förhindrar att SAS skriver ett BOM (Byte Order Mark) tecken när skript-filen skapas	;
	options NOBOMFILE;

	%do %while (%sysfunc(fetch(&dsid)) = 0);
		%let table = %sysfunc(getvarc(&dsid, 1));
		%put TABLE= &table;
		filename textfile "&filepath.&table..sh" termstr=LF;

		* En skriptfil skapas med samma namn som tabellen som ska behörighetstyras.;
		data _null_;
			length commando $400.;
			file textfile encoding='utf-8';
			put "#!/bin/bash -p";
	...
	data work.loggen;
		infile "&filepath.&table..log" dsd truncover encoding='utf-8';
		length Loggfilen $ 5000;
		input Loggfilen $;

		* Om allt gått bra är loggfilen tom, och då kommer inte felmeddelandet nedan skrivas ut.;
		put "ERROR: " Loggfilen;
	run;

	proc print data=work.loggen noobs;
	run;
...
%* setRowSecurity(PATH=%nrstr(/LUL/Folktandvården/Ftv Kansli_Ledning_Adminstration/Data/),
				TABLE=FTV_LASR_T4Clean_FredrikH_testar);
